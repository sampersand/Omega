...
#         if __debug__:
#             assert not isinstance(base, oldomobj), base
#         self.base = base
#         self.evalfunc = evalfunc
#     @staticmethod
#     def genobj(base, evalfunc = None):
#         if isinstance(base, oldomobj):
#             if __debug__:
...
#             return oldomobj(base, evalfunc)
#         try:
#             return oldomobj(eval(base), evalfunc)
#         except SyntaxError:
#             return oldomobj(str(base), evalfunc)
#         except NameError:
#             return oldomobj(str(base), evalfunc)
...
#     def __repr__(self):
#         return 'oldomobj(' + repr(self.base) + ('' if self.evalfunc == None else ', {}'.format(repr(self.evalfunc))) + ')'
...
#         if __debug__:
#             assert isinstance(other, oldomobj), repr(other)
...
#         assert isinstance(locls['$'], oldgroup), repr(locls['$'])
#         return locls['$']
...
#         if __debug__:
#             assert isinstance(eles[not direc], oldgroup), repr(eles[not direc])
#         valueobj = eles[not direc].base.eval(eles, locls)
#         if __debug__:
#             assert isinstance(valueobj, oldgroup)
#         if keystr not in locls:
#             import copy
#             locls[keystr] = copy.copy(valueobj) #oh boy this was giving issues...
#         elif isinstance(locls[keystr], oldgroup): #used to be obj base
...
#                 name = eles.basestr
#                 for ele in eles:
#                     ret.append(oldgroup(base = ele).eval(locls))
#                 locls['$'] = oldgroup(base = ret)
#             elif name in control.delims['endline'][0]:
#                 for ele in eles:
#                     oldgroup(base = ele).eval(locls) #will set locls by itself
#             else:
...
#                     locls['$'] = oldgroup(base = eles)
#                 elif str(eles.base) == str(control.funcs['abort']): 
#                     locls['$'] = oldgroup(base = null())
...
#             oldgroup(base = eles[0]).eval(locls)
